...
- `docs/config/README.md` ✅ REQUIRED (if modifying config)
...
- docs/config/README.md ✅ (if applicable)
...
Scope:
- This rule applies to chat responses, code comments, documentation, PR descriptions, and commit messages generated by AI assistants.
...
### 6. Performance and Reliability Validation (NON-NEGOTIABLE)

AI assistants MUST add and run performance and reliability checks where applicable.

Required standards:
- Add benchmarks (`*_test.go` with `Benchmark...`) for performance-sensitive code paths.
- Validate benchmark impact before and after non-trivial changes.
- Check and report test coverage for modified packages and critical paths.
- Increase coverage when adding new public behavior, bug fixes, or concurrency logic.
- Always review concurrency code for potential deadlocks.
- Always review long-lived processes and high-allocation paths for memory leaks.

Benchmark requirements:
- Use Go benchmark functions (`func BenchmarkXxx(b *testing.B)`).
- Benchmark realistic workloads and input sizes.
- Avoid micro-benchmarks that do not reflect real usage patterns.
- Document benchmark results in the response when benchmarks were added or changed.

Coverage requirements:
- Run coverage checks for affected packages.
- Prefer focused coverage on changed/critical code over broad low-value tests.
- Treat missing tests for new behavior as non-compliant unless explicitly approved.
...
Minimum validation commands (run when applicable):
```bash
go test ./...
go test -cover ./...
go test -race ./...
...
Tooling guidance (when applicable):
- `pprof` for heap/cpu profiling
- race detector (`-race`) for concurrent access issues
- benchmark memory metrics (`-benchmem`) for allocation regressions

Response requirements:
- Include benchmark and coverage results when relevant.
- State deadlock/memory-leak review outcome explicitly.
- If checks were not applicable, explain why.
...
11. **ALWAYS add benchmarks where applicable** - Especially for performance-sensitive paths
12. **ALWAYS check coverage for changed code** - Add tests when behavior changes
13. **ALWAYS review for deadlocks** - Validate concurrency safety before finalizing
14. **ALWAYS review for memory leaks** - Validate resource lifecycle and allocation behavior
